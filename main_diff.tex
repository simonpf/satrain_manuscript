...
%DIF DEL main.tex           Sun Nov 23 14:22:08 2025
%DIF ADD main_revised.tex   Sun Nov 23 23:55:13 2025
...
\DIFaddend \begin{abstract}
  ...
  essential for both research and operational meteorology. Satellite
  observations remain the only means of achieving consistent, global-scale
  precipitation monitoring. While machine learning has long been applied to
  satellite-based precipitation retrieval, the absence of a standardized
  benchmark dataset has hindered fair comparisons between methods and limited
...
estimation of rain, snow, graupel, and hail. SatRain \DIFdelbegin \DIFdel{includes }\DIFdelend \DIFaddbegin \DIFadd{integrates }\DIFaddend multi-sensor
...
States}\DIFaddend . It offers a standardized evaluation protocol \DIFaddbegin \DIFadd{as well as
out-of-distribution testing data from Asia and Europe }\DIFaddend to enable robust and
...
with \DIFdelbegin \DIFdel{passive microwave }\DIFdelend \DIFaddbegin \DIFadd{PMW }\DIFaddend sensors is limited, requiring deployment in low-Earth
...
tropics\DIFdelbegin \DIFdel{(30 °S - 30 °N)}\DIFdelend .
...
Panel (b) displays thermal \DIFdelbegin \DIFdel{infrared (IR ) }\DIFdelend \DIFaddbegin \DIFadd{IR }\DIFaddend imagery with a wavelength of
...
precipitation estimates underscores the strength of \DIFdelbegin \DIFdel{passive microwave
}\DIFdelend \DIFaddbegin \DIFadd{PMW
}\DIFaddend observations: while \DIFdelbegin \DIFdel{visible }\DIFdelend \DIFaddbegin \DIFadd{Vis }\DIFaddend and IR imagery primarily depict cloud-top features
that may not correlate with surface rainfall, \DIFdelbegin \DIFdel{microwave }\DIFdelend \DIFaddbegin \DIFadd{PMW }\DIFaddend observations exhibit
higher spatial coherence with surface precipitation. The principal limitation of
\DIFdelbegin \DIFdel{passive microwave }\DIFdelend \DIFaddbegin \DIFadd{PMW }\DIFaddend imagery, however, is its limited spatiotemporal coverage due
...
\DIFaddend While a physics-based formulation of precipitation retrieval algorithms is
...
\DIFaddend \subsection{The Need for a Unified Benchmark Dataset}
...
large, curated training set over the \DIFdelbegin \DIFdel{continental }\DIFdelend \DIFaddbegin \DIFadd{conterminous }\DIFaddend United States (CONUS),
...
The SatRain dataset integrates \DIFdelbegin \DIFdel{four principal }\DIFdelend \DIFaddbegin \DIFadd{the following }\DIFaddend data sources:

\begin{enumerate}
	\item \DIFdelbegin \DIFdel{Passive microwave }\DIFdelend \DIFaddbegin \DIFadd{PMW }\DIFaddend observations,
	\item \DIFdelbegin \DIFdel{Visible and infrared }\DIFdelend \DIFaddbegin \DIFadd{Vis and IR }\DIFaddend observations from geostationary platforms,
...
spatial resolution and sampling across sensors \DIFaddbegin \DIFadd{and datasets}\DIFaddend . To balance flexibility with
...
operational retrievals such as \DIFdelbegin \DIFdel{the Goddard Profiling Algorithm
\mbox{%DIFAUXCMD
\citep{Kummerow2015GPROF}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{GPROF}\DIFaddend .
...
\begin{figure}[htbp] % h=here, t=top, b=bottom, p=page of floats
	\centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=1.0\textwidth]{figures/fig02}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=1.0\textwidth]{figures_revised/fig02.pdf}
	\DIFaddendFL \caption{Passive microwave observations from the GMI (a - c) and ATMS (d - f) sensors from the landfall of Hurricane Laura on August 27, 2020. The top row displays three selected channels from the GMI sensor, highlighting its high-resolution but limited swath. The bottom row presents corresponding channels from the ATMS sensor for comparison. The blue contour line encloses regions with surface precipitation rates exceeding 1 mm h$^{-1}$.}
...
\DIFdelbegin \DIFdel{sFigure}\DIFdelend \DIFaddbegin \DIFadd{Figure}\DIFaddend ~\ref{fig:observations_pmw} compares GMI and ATMS observations of
...
near-circular \DIFdelbegin \DIFdel{footprints }\DIFdelend \DIFaddbegin \DIFadd{scans on the surface }\DIFaddend at a nearly constant Earth-incidence angle. This
...
scan.
\DIFdelbegin \DIFdel{ystematic differences across the scan.
}\DIFdelend 
...
\begin{table}[htbp]
	...
	\DIFaddbeginFL \caption{
		\DIFaddFL{Frequencies (Freq.), polarizations (Pol.), and footprint (FP) sizes of the
	...
	\DIFaddendFL \begin{subtable}{0.45\textwidth}
		\centering
		\begin{tabular}{ccc}
...
{%DIFAUXCMD
\DIFdelFL{Frequencies (Freq.), polarizations (Pol.), and footprint (FP) sizes of the
		PMW observations included in the SatRain dataset. For GMI, footprint sizes are
		reported as the full width at half maximum (FWHM) along and across the
		boresight direction. For ATMS, only the nadir FWHM is given; at nadir the
		footprint is circular and therefore represented by a single value. The
	...
	%DIFAUXCMD
%DIFDELCMD < \label{tab:pmw_channels}
%DIFDELCMD < %%%
...
than that of PMW sensors. The first six channels are \DIFdelbegin \DIFdel{visible and near-infrared
}\DIFdelend \DIFaddbegin \DIFadd{VIs and near-IR
}\DIFaddend bands that measure reflected solar radiation and are only available during
...
global precipitation retrievals is that the \DIFdelbegin \DIFdel{channels }\DIFdelend \DIFaddbegin \DIFadd{channel }\DIFaddend availability changes between
\DIFdelbegin \DIFdel{platforms }\DIFdelend \DIFaddbegin \DIFadd{sensors }\DIFaddend and thus the geographical coverage regions. SatRain integrates data from
CONUS, the Korean peninsula, and Austria, which are covered by different
geostationary platforms. While observations over CONUS are derived from the \DIFdelbegin \DIFdel{GOES 16 }\DIFdelend \DIFaddbegin \DIFadd{GOES-16
}\DIFaddend \citep{Goodman2019Goes} platform, observations over Korea are derived from
the \DIFdelbegin \DIFdel{Himawari 8 and 9 }\DIFdelend \DIFaddbegin \DIFadd{Himawari-8 and -9 }\DIFaddend platforms \citep{Bessho2016Himawari}, and observations over
Austria from the \DIFdelbegin \DIFdel{Meteosat 10 }\DIFdelend \DIFaddbegin \DIFadd{Meteosat-10 }\DIFaddend platform \citep{Schmetz2002Introduction}.
Table~\ref{tab:geo_channels} lists the central wavelengths \DIFdelbegin \DIFdel{and corresponding bandwidths }\DIFdelend for the channels
included from the ABI sensor on \DIFdelbegin \DIFdel{GOES 16}\DIFdelend \DIFaddbegin \DIFadd{GOES-16}\DIFaddend , the Advanced Himaware Imager (AHI,
\citeauthor{Da2015Preliminary}, \citeyear{Da2015Preliminary}), and the SEVIRI
\citep{Aminou2002Msg} sensor on \DIFdelbegin \DIFdel{Meteosat 10. }\DIFdelend \DIFaddbegin \DIFadd{Meteosat-10. }\DIFaddend The observations from the ABI, AHI,
...
\begin{table}[htbp]
  \centering
	\DIFaddbeginFL \caption{\DIFaddFL{Channel central wavelengths ABI sensor on GOES-16, the AHI sensor on Himawari-8/9, and the SEVIRI sensor on Meteosat-10.}}
...
\DIFdelbeginFL %DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Channel central wavelengths ABI sensor on GOES-16, the AHI sensor on Himawari-8/9, and the SEVIRI sensor on Meteosat-10.}}
	%DIFAUXCMD
%DIFDELCMD < \label{tab:geo_channels}
%DIFDELCMD < %%%
\DIFdelendFL \end{table}
...
\begin{figure}[htbp] % h=here, t=top, b=bottom, p=page of floats
	\centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=1.0\textwidth]{figures/fig04}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=1.0\textwidth]{figures_revised/fig04.pdf}
	\DIFaddendFL \caption{Ancillary variables provided by the SatRain dataset for the scene depicted in Fig. 1. Panels (a) to (q) show the dynamic ERA5 fields included in the ancillary data. Panel (r ) shows the 18-class surface classification from the GPROF algorithm and panel (s) the surface elevation from the NOAA Globe dataset.}
...
that has been developed for the \DIFdelbegin \DIFdel{Goddard Profiling Algorithm (GPROF ,
\mbox{%DIFAUXCMD
\citeauthor{GPM_GPROF_ATBD_V7}}\hspace{0pt}%DIFAUXCMD
, \mbox{%DIFAUXCMD
\citeyear{GPM_GPROF_ATBD_V7}}\hspace{0pt}%DIFAUXCMD
) precipitation retrieval  }\DIFdelend \DIFaddbegin \DIFadd{GPROF precipitation retrieval  \mbox{%DIFAUXCMD
\citep{GPM_GPROF_ATBD_V7}
}\hspace{0pt}%DIFAUXCMD
}\DIFaddend combing microwave-based surface-type information with snow- and
...
}\DIFdelend showcases the ancillary variables included in the SatRain dataset for the
...
\begin{figure}[htbp] % h=here, t=top, b=bottom, p=page of floats
	\centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=1.0\textwidth]{figures/fig05}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=1.0\textwidth]{figures_revised/fig05.pdf}
	\DIFaddendFL \caption{Reference precipitation estimates and auxiliary fields \DIFdelbeginFL \DIFdelFL{dataset }\DIFdelendFL during the landfall of Hurricane Laura. Panel (a) shows surface precipitation estimates from NOAA’s gauge-corrected Multi-Radar Multi-Sensor product. Panel (b) displays the radar quality index quantifying the reliability of the precipitation estimates. Panel (c) presents the convective fraction field, illustrating the hydrometeor classification data included in the SatRain dataset.}
	\label{fig:observations_geo}
\end{figure}
...
\DIFdelbegin \DIFdel{Nexrad}\DIFdelend \DIFaddbegin \DIFadd{NEXRAD}\DIFaddend , the most extensive network of precipitation radars in the world
...
\begin{figure}[htbp] % h=here, t=top, b=bottom, p=page of floats
	\centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=1.0\textwidth]{figures/fig06}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=1.0\textwidth]{figures_revised/fig06.pdf}
	\DIFaddendFL \caption{Independent test data included in the SatRain dataset. Panel (a) shows the spatial coverage of the three temporally and spatially independent test datasets. Panel (b) shows the location of the WegenerNet gauge stations used as reference measurements in the Austria domain and the grid they are aggregated to. Panel (c) shows an example in the Korea domain, with radar and rain-gauge data obtained from the Korea Meteorological Administration.}
	\label{fig:observations_geo}
\end{figure}

The SatRain training data is derived from four years (2018–2021) of ground-based
radar measurements over CONUS. While this region encompasses a \DIFdelbegin \DIFdel{relatively }\DIFdelend wide range of
...
that radar \DIFdelbegin \DIFdel{/}\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend gauge networks still struggle with. For the comparison against
...
independent. Training and validation data are extracted from the years \DIFdelbegin \DIFdel{2019
...
\begin{figure}[htbp]
	\centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=1.0\textwidth]{figures/fig07}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=1.0\textwidth]{figures_revised/fig07.pdf}
	\DIFaddendFL \caption{Flow diagram illustrating the data flow for collocating the satellite observations and ground-based reference precipitation estimates for the SatRain dataset.}
	\label{fig:data_flow}
\end{figure}
...
64$ used for the \DIFdelbegin \DIFdel{ATMS }\DIFdelend on-swath data is \DIFdelbegin \DIFdel{due to lower across-track sampling rate
of ATMS, which }\DIFdelend \DIFaddbegin \DIFadd{to accomodate both the GMI sensor, which has
221 pixels per scan, and the ATMS sensor which }\DIFaddend records only 96 pixel per \DIFdelbegin \DIFdel{scans as opposed to 221 for GMI}\DIFdelend \DIFaddbegin \DIFadd{scan}\DIFaddend .
...
\DIFdelbegin \DIFdel{(Plate
Carrée) }\DIFdelend projection used for the figures. \DIFaddbegin \DIFadd{Training patches are restricted to the PMW
...
\DIFaddbegin \url{rain.atmos.colostate.edu/ipwgml} \DIFaddend \citep{SatRain2025}. Additionally,
...
\begin{figure}[hbpt!] % h=here, t=top, b=bottom, p=page of floats
	...
	\DIFdelbeginFL %DIFDELCMD < \dirtree{%
%DIFDELCMD < 		.1 satrain/.
%DIFDELCMD < 		.2 \textlangle base sensor\textrangle.
%DIFDELCMD < 		.3 \textlangle split \textrangle.
%DIFDELCMD < 		.4 xs.
%DIFDELCMD < 		.5 gridded.
%DIFDELCMD < 		.6 \textlangle year\textrangle /\textlangle month\textrangle /\textlangle day\textrangle .
%DIFDELCMD < 		.5 on\_swath.
%DIFDELCMD < 		.6 \textlangle year\textrangle /\textlangle month\textrangle /\textlangle day\textrangle.
%DIFDELCMD < 		.4 ....
%DIFDELCMD < 		.4 xl.
%DIFDELCMD < 		.5 gridded.
...
A further distinction between the training and validation data  \DIFdelbegin \DIFdel{on one hand }\DIFdelend and
the testing data  \DIFdelbegin \DIFdel{on the other }\DIFdelend is that the collocation scenes in the testing set
are not divided into fixed-size patches. The testing data preserves the original
observation structure \DIFdelbegin \DIFdel{is }\DIFdelend to avoid sampling distortions, simplify comparisons
...
and target data to train and evaluate a retrieval using only \DIFdelbegin \DIFdel{passive microwave
}\DIFdelend \DIFaddbegin \DIFadd{PMW }\DIFaddend observations.
Each file is identified using an individual prefix ('\texttt{gmi\_}',
'\texttt{atms\_}', '\texttt{geo\_}', '\texttt{geo\_ir\DIFaddbegin \DIFadd{\_}\DIFaddend }',
...
brightness temperatures. The geostationary data currently \DIFdelbegin \DIFdel{does }\DIFdelend \DIFaddbegin \DIFadd{do }\DIFaddend not include
...
observations from four 10-minute time steps prior to the collocation median \DIFaddbegin \DIFadd{time }\DIFaddend and
...
from \DIFaddbegin \DIFadd{the }\DIFaddend AHI sensor onboard \DIFdelbegin \DIFdel{Himawari 8 and 9 }\DIFdelend \DIFaddbegin \DIFadd{Himawari-8 and -9 }\DIFaddend and the SEVIRI sensor onboard \DIFdelbegin \DIFdel{Meteosat
10. }\DIFdelend \DIFaddbegin \DIFadd{Meteosat-10.
}\DIFaddend Table~\ref{tab:geo_channels} lists the central wavelengths of the
...
single-timestep files contain the \DIFdelbegin \DIFdel{GEO }\DIFdelend \DIFaddbegin \DIFadd{Geo }\DIFaddend IR observations closest to the
measurement time of the reference precipitation estimates. The
temporally-resolved \DIFdelbegin \DIFdel{GEO }\DIFdelend \DIFaddbegin \DIFadd{Geo }\DIFaddend IR observations contain 16 half-hourly observations
...
\begin{table}[htbp]
	\centering
	\DIFaddbeginFL \caption{\DIFaddFL{List of the variables, their explanations, units, and data sources included in the ancillary data.}}
	\label{tab:ancillary}
	\DIFaddendFL \begin{tabular}{lp{4cm}ll}
		\toprule
...
\DIFdelbeginFL %DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{List of the variables, their explanations, units, and data sources included in the ancillary data.}}
	%DIFAUXCMD
%DIFDELCMD < \label{tab:ancillary}
%DIFDELCMD < %%%
\DIFdelendFL \end{table}
...
one of the primary input observation types: \DIFdelbegin \DIFdel{multi-channel geostationary
measurements from GOES ABI (GEO), }\DIFdelend single-channel \DIFdelbegin \DIFdel{infrared }\DIFdelend \DIFaddbegin \DIFadd{IR }\DIFaddend data from the
CPCIR dataset (\DIFdelbegin \DIFdel{GEO-IR), and passive microwave }\DIFdelend \DIFaddbegin \DIFadd{Geo-IR), multi-channel Vis and IR geostationary observations from
the GOES ABI (Geo), and PMW }\DIFaddend observations from GMI (GMI). \DIFdelbegin \DIFdel{These
models were then applied to estimate precipitation during the landfall of
Typhoon Khanun over Korea. }\DIFdelend All retrievals rely
solely on satellite observations and exclude the ancillary data included in
SatRain. Each retrieval is implemented \DIFdelbegin \DIFdel{using a UNet architecture built on }\DIFdelend \DIFaddbegin \DIFadd{U-Net-type encoder-decoder based on the
}\DIFaddend EfficientNet-V2 \DIFdelbegin \DIFdel{with 10 }\DIFdelend \DIFaddbegin \DIFadd{architecture \mbox{%DIFAUXCMD
\citep{tan2021efficientnetv2} }\hspace{0pt}%DIFAUXCMD
with 14 }\DIFaddend million
parameters.

\DIFaddbegin \DIFadd{The SatRain-based ML retrievals are evaluated against two baseline precipitation
...
\subsection{\DIFadd{Case Studies}}

\DIFadd{We first demonstrate retrieval results for two case studies from the test
data of the SatRain dataset: A convective squall line over western CONUS and the
landfall of Typhoon Khanun over Korea.
}

\DIFaddend \begin{figure}[htbp] % h=here, t=top, b=bottom, p=page of floats
	\centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=1.0\textwidth]{figures/fig09}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=1.0\textwidth]{figures_revised/fig11.pdf}
	\DIFaddendFL \caption{ Precipitation retrievals \DIFaddbeginFL \DIFaddFL{for a quasi-linear convective system, comparing models }\DIFaddendFL trained \DIFdelbeginFL \DIFdelFL{using }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{on }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{CONUS-based }\DIFdelendFL SatRain \DIFdelbeginFL \DIFdelFL{training }\DIFdelendFL dataset \DIFdelbeginFL \DIFdelFL{compared to }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{with }\DIFaddendFL ground-based precipitation estimates and two baseline \DIFdelbeginFL \DIFdelFL{datasets during the landfall of Typhoon Khanun over the Korean
		peninsula}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{products}\DIFaddendFL . Panel (a) shows the ground-based reference precipitation\DIFdelbeginFL \DIFdelFL{estimates}\DIFdelendFL , panel (b) \DIFdelbeginFL \DIFdelFL{shows the precipitation field from }\DIFdelendFL the ERA5 reanalysis, \DIFaddbeginFL \DIFaddFL{and }\DIFaddendFL panel (c) \DIFdelbeginFL \DIFdelFL{shows precipitation estimates from }\DIFdelendFL the GPROF \DIFdelbeginFL \DIFdelFL{precipitation
		}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{V7 }\DIFaddendFL retrieval applied to GMI observations. Panels (d), (e), and (f) \DIFdelbeginFL \DIFdelFL{show
		}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{present SatRain-trained }\DIFaddendFL retrievals \DIFdelbeginFL \DIFdelFL{trained using the SatRain dataset over CONUS }\DIFdelendFL using \DIFaddbeginFL \DIFaddFL{single-channel IR observations (Geo-IR), }\DIFaddendFL multi-channel Vis \DIFdelbeginFL \DIFdelFL{/}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{and }\DIFaddendFL IR \DIFdelbeginFL \DIFdelFL{observation, single-channel GEO-IR }\DIFdelendFL observations \DIFaddbeginFL \DIFaddFL{(Geo)}\DIFaddendFL , and GMI PMW observations, respectively. \DIFdelbeginFL \DIFdelFL{The dashed line marks the bounadary of the GMI
		swath.
	}\DIFdelendFL }
	\DIFdelbeginFL %DIFDELCMD < \label{fig:case_study}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \label{fig:case_study_1}
\DIFaddendFL \end{figure}

\DIFdelbegin \DIFdel{The retrieved precipitation fields are compared to the
reference ground-based
radar estimates and two baselines from the ERA-5 reanalysis dataset and }\DIFdelend \DIFaddbegin \DIFadd{Figure \ref{fig:case_study_1} presents the reference and retrieved surface
...
~\ref{fig:case_study}. The results
...
\begin{figure}[htbp] %DIF >  h=here, t=top, b=bottom, p=page of floats
	\centering
	\includegraphics[width=1.0\textwidth]{figures_revised/fig12.pdf}
	\caption{\DIFaddFL{Same layout as Figure \ref{fig:case_study_1}, but for a case depicting the landfall of Typhoon Khanun over the Korean Peninsula on August 9, 2023, taken from the Korea test-domain dataset.}}
	\label{fig:case_study_2}
\end{figure}

\DIFadd{Figure~\ref{fig:case_study_2} compares the same retrievals on a test scene from
the Korea domain depicting the landfall }\DIFaddend of Typhoon Khanun \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{on August 9, 2023. All
estimates capture the large-scale precipitation features of the Typhoon }\DIFaddend but with
...
bands but misses finer details resolved by \DIFdelbegin \DIFdel{PMW}\DIFdelend \DIFaddbegin \DIFadd{the PMW retrieval}\DIFaddend . In contrast,
...
\DIFaddbegin \subsection{\DIFadd{Test-Set Accuracy}}

\DIFaddend \begin{figure}[htbp] % h=here, t=top, b=bottom, p=page of floats
	\centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=1.0\textwidth]{figures/fig10}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=1.0\textwidth]{figures_revised/fig13.pdf}
	\DIFaddendFL \caption{Evaluation of quantitative precipitation estimates from two state-of-the-art precipitation datasets (ERA5 and GPROF) and \DIFdelbeginFL \DIFdelFL{ML retrievals }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{a U-Net retrieval }\DIFaddendFL trained using the different satellite observations of the SatRain dataset. Each retrieval is evaluated for each of the three testing domains of the SatRain dataset. Panels (a) to (d) show the Bias, mean-absolute error (MAE), mean-squared error (MSE), and linear correlation coefficient, respectively.}
...
Austria. As shown }\DIFaddend in Fig.~\ref{fig:sensor_comparison}, \DIFdelbegin \DIFdel{are consistent }\DIFdelend \DIFaddbegin \DIFadd{the results align }\DIFaddend with
...
but also affect operational \DIFdelbegin \DIFdel{products }\DIFdelend \DIFaddbegin \DIFadd{algorithms }\DIFaddend such as GPROF \DIFaddbegin \DIFadd{V7}\DIFaddend . Beyond these biases, the
...
One exception occurs for the \DIFdelbegin \DIFdel{GEO }\DIFdelend \DIFaddbegin \DIFadd{Geo }\DIFaddend retrieval over Austria, where MSE and MAE
...
a multi-layer perceptron (MLP), and \DIFdelbegin \DIFdel{a convolutional neural network (CNN)}\DIFdelend \DIFaddbegin \DIFadd{the U-Net architecture assessed above}\DIFaddend . Their
performance, shown in Fig.~\DIFdelbegin \DIFdel{\ref{fig:sensor_comparison}}\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:algorithm_comparison}}\DIFaddend , reveals a consistent
ranking across both sensors and all three domains \DIFdelbegin \DIFdel{: CNN-based models
clearly outperform }\DIFdelend \DIFaddbegin \DIFadd{with the U-Net-based models
clearly outperforming }\DIFaddend the other techniques. Although the relative ordering of
...
\begin{figure}[htbp]
	\centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=1.0\textwidth]{figures/fig11}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=1.0\textwidth]{figures_revised/fig14.pdf}
	\DIFaddendFL \caption{Like Fig.\DIFdelbeginFL \DIFdelFL{8 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{~\ref{fig:sensor_comparison} }\DIFaddendFL but for four different machine-learning techniques trained on GMI (first row) and ATMS (second row) observations.}
	\DIFdelbeginFL %DIFDELCMD < \label{fig:sensor_comparison}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \label{fig:algorithm_comparison}
\DIFaddendFL \end{figure}

\DIFdelbegin \DIFdel{Taken together, these results underscore the value of SatRain as a benchmark
dataset. It }\DIFdelend \DIFaddbegin \DIFadd{We also evaluated the ability of the SatRain-based U-Net retrievals to detect precipitation and
...
SatRain dataset }\DIFaddend offers a well-defined framework for training and evaluation \DIFdelbegin \DIFdel{that
enables }\DIFdelend \DIFaddbegin \DIFadd{thus
enabling }\DIFaddend direct comparison of different ML techniques. The independent test data
...
precipitation. We adopt thresholds of \DIFdelbegin \DIFdel{0.1 mm/h and
10 mm/h }\DIFdelend \DIFaddbegin \DIFadd{$\SI{0.1}{\milli \meter \per \hour}$ and
$\SI{10}{\milli \meter \per \hour}$ }\DIFaddend to define precipitation and heavy
...
their models on all or a subset of the defined tasks. \DIFdelbegin \DIFdel{For
each task, the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend recommended accuracy
metrics \DIFaddbegin \DIFadd{for each task }\DIFaddend are listed in Table 4.
...
criteria. \DIFdelbegin \DIFdel{By providing a simple interface function, users can }\DIFdelend \DIFaddbegin \DIFadd{A well-defined interface allows all users to }\DIFaddend leverage the package to
automatically conduct evaluations across all testing domains, tasks, and
metrics. The \texttt{satrain} package \DIFdelbegin \DIFdel{thus offers a fast, consistent, and reproducible framework for evaluating precipitation retrieval algorithms}\DIFdelend \DIFaddbegin \DIFadd{also provides automated tiling
functionality to ease the transition from the fixed-size scenes of the training
and validation data to the variable-size overpasses used for the testing data}\DIFaddend .
...
The precipitation reference data are also subject to \DIFdelbegin \DIFdel{significant uncertainties. While }\DIFdelend \DIFaddbegin \DIFadd{non-negligible
uncertainties. Although }\DIFaddend gauge-corrected, ground-based radar composites are
widely \DIFdelbegin \DIFdel{regarded as }\DIFdelend \DIFaddbegin \DIFadd{considered }\DIFaddend the most reliable spatially continuous precipitation estimates
currently available, they are not \DIFdelbegin \DIFdel{error-free}\DIFdelend \DIFaddbegin \DIFadd{without error}\DIFaddend . Beam overshooting,
uncertainties in microphysical \DIFdelbegin \DIFdel{processes}\DIFdelend \DIFaddbegin \DIFadd{assumptions}\DIFaddend , and the \DIFdelbegin \DIFdel{particular difficulty of
estimating snowfall }\DIFdelend \DIFaddbegin \DIFadd{inherent difficulty of
quantifying snowfall all }\DIFaddend introduce systematic biases. Snowfall \DIFdelbegin \DIFdel{poses a notable challenge}\DIFdelend \DIFaddbegin \DIFadd{is particularly
...
facilitating \DIFaddbegin \DIFadd{and advancing }\DIFaddend the operational adoption of ML-based retrievals. It
...
multi-sensor, multi-timestep design provides a \DIFdelbegin \DIFdel{strong foundation for developing
}\DIFdelend \DIFaddbegin \DIFadd{foundation for testing
}\DIFaddend next-generation algorithms that fuse observations across sensors and time steps
...
\DIFaddbegin \DIFadd{Although SatRain was developed to support the testing of basic multi-sensor
...
\DIFaddend The SatRain project has been developed as a community effort to provide both an
...
%DIFDELCMD <  %%%
\DIFdelend \DIFaddbegin \bibliography{embedded_refs}
